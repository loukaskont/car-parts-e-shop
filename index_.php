...
<div id="search-logo">
<table style="width:100%; position:fixed; top:24%;">
  <tr>
    ...
    <td align="center">
		<div class="form-wrapper">
			<input style="width:320px; height:40px; align:left;" onblur="searchProducts(1);" onkeyup="searchProductsEnter(event);" type="text" id="searchTextBox" placeholder="Αναζητηση..." required>
...
<br/>
		...
		<div id='cssmenu' style="position:fixed; top:0;">
           <ul>
					...
					<li class='active'><a href='cartPage.php'><input type="text" id="cartProductsCount" value="0 Προϊόντα" style="width:90px; border: none;" readonly></br><input type="text" id="cartSumPrice" value="0 €" style="width:40px; position-right:0px; border: none;" readonly></br><img style="max-width:100%; height:auto;"  src="cart.png" alt="Mountain View"></a>
						<ul id="cartUl">
						</ul>
					</li>
...
{
	echo '<body onscroll="scrollingPage()" onload="clearCart();">';
}
else
{
...
<?php
	...
	echo '<option name="HYUNDAI" onclick="showMontela(75);" value="Hyundai">Hyundai</option>';
	//echo '<option name="TOYOTA" onclick="showMontela(11);" value="TOYOTA">TOYOTA</option>';
	...
	if ($result->num_rows > 0)
	{
		...
		{
			echo '<option style = "display:none" name="'.$row['onoma'].'" onclick="showEkdosis('.$row['id_montelou'].');" id="'.$row['id_markas'].'" value="'.$row['onoma_montelou'].'">'.$row['onoma_montelou'].'</option>';
		}
	}
	echo '</select><br/><br/><br/>';
	...
	if ($result->num_rows > 0)
	{
		while($row = $result->fetch_assoc())
		{
			echo '<option onclick="setIdEkdosisToCookies('.$row['id_ekdosis'].');" name="'.$row['onoma'].'" style = "display:none" id="'.$row['id_montelou'].'" value="'.$row['onoma'].'">'.$row['onoma'].'</option>';
		}
	}
	echo '</select><br/><br/>';
	echo '</div>';
		$insertEpiskepseis = 'INSERT INTO episkepseis(ip, imerominia, products, lastPage) VALUES (\''.$_SERVER['REMOTE_ADDR'].'\',\''.date("Y/m/d h:i:s").'\',\''.$productsStartSeper.'\',\'index.php\');';
...
if(isset($_GET['searchText']))
{
	echo '<div>';
		...
		$sqlArithmos = '';
		$sql = '';
		if(isset($_GET['id_ekdosis']))
		{
			$sqlArithmos = "SELECT  proionta.posostoKerdous, proionta.id as idProiontos, arithmos_proiontos, titlos, timi, id_antalaktika, path_eikonas FROM r105607lou_antalaktika.montelo join ekdosi on montelo.id_montelou = ekdosi.id_montelou join ekdosi_proionta on ekdosi.id_ekdosis = ekdosi_proionta.id_ekdosis join proionta on ekdosi_proionta.id_proiontos = proionta.id join proionta_eikones on proionta.id = proionta_eikones.id_proiontos where ekdosi.id_ekdosis = ".$_GET['id_ekdosis']." and arithmos_proiontos like '%".$_GET['searchText']."%' and isLogoImage = 0 and active = '1';";
			$sql = "SELECT  proionta.posostoKerdous, proionta.id as idProiontos, arithmos_proiontos, titlos, timi, id_antalaktika, path_eikonas FROM r105607lou_antalaktika.montelo join ekdosi on montelo.id_montelou = ekdosi.id_montelou join ekdosi_proionta on ekdosi.id_ekdosis = ekdosi_proionta.id_ekdosis join proionta on ekdosi_proionta.id_proiontos = proionta.id join proionta_eikones on proionta.id = proionta_eikones.id_proiontos where ekdosi.id_ekdosis = ".$_GET['id_ekdosis']." and titlos like '%".$_GET['searchText']."%' and isLogoImage = 0 and active = '1';";
		}
		else
		{
			$sqlArithmos = "SELECT  proionta.posostoKerdous, proionta.id as idProiontos, arithmos_proiontos, titlos, timi, id_antalaktika, path_eikonas FROM r105607lou_antalaktika.montelo join ekdosi on montelo.id_montelou = ekdosi.id_montelou join ekdosi_proionta on ekdosi.id_ekdosis = ekdosi_proionta.id_ekdosis join proionta on ekdosi_proionta.id_proiontos = proionta.id join proionta_eikones on proionta.id = proionta_eikones.id_proiontos where arithmos_proiontos like '%".$_GET['searchText']."%' and isLogoImage = 0 and active = '1' limit 500;";
			$sql = "SELECT  proionta.posostoKerdous, proionta.id as idProiontos, arithmos_proiontos, titlos, timi, id_antalaktika, path_eikonas FROM r105607lou_antalaktika.montelo join ekdosi on montelo.id_montelou = ekdosi.id_montelou join ekdosi_proionta on ekdosi.id_ekdosis = ekdosi_proionta.id_ekdosis join proionta on ekdosi_proionta.id_proiontos = proionta.id join proionta_eikones on proionta.id = proionta_eikones.id_proiontos where titlos like '%".$_GET['searchText']."%' and isLogoImage = 0 and active = '1' limit 500;";
		}
		$result = $conn->query($sql);
		if ($result->num_rows == 0)
		{
			$resultArithmos = $conn->query($sqlArithmos);
			if($resultArithmos->num_rows > 0)
			{
				$result = $resultArithmos;
			}
		}
		if ($result->num_rows > 0)
		{
			...
			while($row = $result->fetch_assoc())
			{
				if (!in_array($row['timi'], $proiontaArray)) 
				{
					...
					<div class="cd-single-item">
						<a id="productDetailsLink" href="productDetails.php?idProiontos='.$row['idProiontos'].'">
