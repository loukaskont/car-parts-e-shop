...
<br/>
		...
		<div id='cssmenu'style="position:fixed; top:0;">
           <ul>
					...
					<li class='active'><a href='cartPage.php'><span><input type="text" id="cartProductsCount" value="0 προιόντα" style="width:100; border: none;" readonly></br><input type="text" id="cartSumPrice" value="" style="width:100; border: none;" readonly><img style="max-width:100%; height:auto;"  src="cart.png" alt="Mountain View"></span></a>
						<ul id="cartUl">
						</ul>
					</li>
			</ul>
...
<?php
echo '<body onscroll="scrollingPage()" onload="loadCart()">';
?>

</br>
<div>
<?php
	if(isset($_GET["idProiontos"]))
	{
	$_SESSION['idProiontos'] = $_GET["idProiontos"];
	...
	$sql = "SELECT  proionta.posostoKerdous, arithmos_proiontos, titlos, timi, id_antalaktika, path_eikonas, xaraktiristika FROM ****.montelo join ekdosi on montelo.id_montelou = ekdosi.id_montelou join ekdosi_proionta on ekdosi.id_ekdosis = ekdosi_proionta.id_ekdosis join proionta on ekdosi_proionta.id_proiontos = proionta.id join proionta_eikones on proionta.id = proionta_eikones.id_proiontos where proionta.id = ".$_GET["idProiontos"]." and isLogoImage = 0 and active = '1';";
	...
	if ($result->num_rows > 0)
	{
		...
		{
			echo '<p style = "font-family: serif; color:#00bfff; font-size: 25px;">';
			echo $row['titlos'];
			if(isset($_SESSION['monteloName']))
			{
				echo ' για '.$_SESSION['monteloName'];
			}
			echo '</p>';
			echo '</br></br></br></br>';
			if (!in_array($row['timi'], $proiontaArray)) 
			{
				...
				$proiontaArray[$proiontaIndex] = $telikiTimi;
				$proiontaIndex++;
				echo'<li>
				<div class="cd-single-item">
					<a href="#0">
						<ul class="cd-slider-wrapper">
							<li class="selected"><img style="max-width:100%; height:auto;" src="';
				echo 'images\\products\\'.$row['path_eikonas'];
				echo '" alt="Preview image"></li>
						</ul>
					</a>
					<div class="cd-customization">
						<button class="add-to-cart" onclick="addToCart(\''.$_GET["idProiontos"].'#'.$row['titlos'].'#'.$row['path_eikonas'].'#'.$telikiTimi.'\')">
							<em>στο καλαθι</em>
				...
				echo '</a></b>
					<em>';
				echo '<p style = "font-family: serif; color:#00bfff; font-size: 25px;">';
				echo number_format((float)$row['timi'] + $row['timi'] * $row['posostoKerdous'] / 100, 2, '.', '');
				echo '</p>';
				echo '</em> </div> </li>';
				echo '<li>';
				echo '<div>';
				echo '<u>ΠΕΡΙΓΡΑΦΗ ΠΡΟΙΟΝΤΟΣ:</u></br></br>';
				echo '<p style = "font-family: serif;">';
				echo str_replace("#","</br>",$row['xaraktiristika']);
				echo '</p>';
				echo '</div>';
				echo '</li>';
			}
	...
	$insertEpiskepseis = 'INSERT INTO episkepseis(ip, imerominia, products, lastPage) VALUES (\''.$_SERVER['REMOTE_ADDR'].'\',\''.date("Y/m/d h:i:s").'\',\''.$_GET["idProiontos"].'\',\'productDetails.php_'.$_GET["idProiontos"].'\');';
